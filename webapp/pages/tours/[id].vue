  ...
  <div v-if="tour" class="container-site py-6 max-w-5xl">
    <div class="grid lg:grid-cols-[1fr_360px] gap-8">
      <div>
        ...
        <p class="text-ink-muted text-sm mt-1">{{ tour.city }} · {{ tour.durationHours }} jam</p>
        ...
        <div v-if="tour.highlights?.length" class="card p-5 mt-4">
          <h2 class="font-bold text-lg mb-2">Highlight</h2>
        ...
        <div v-if="tour.included?.length" class="card p-5 mt-4">
          <h2 class="font-bold text-lg mb-2">Termasuk</h2>
      ...
      <aside class="lg:sticky lg:top-20 h-fit">
        <div class="card p-5">
          <div class="text-3xl font-extrabold text-brand-700">{{ rupiah(tour.price) }}<span class="text-sm font-normal text-ink-faint">/orang</span></div>
          <div v-if="isLoggedIn && !doneCode" class="mt-4 space-y-3">
            <div><label class="label">Tanggal</label><input v-model="date" type="date" :min="todayStr" class="input" /></div>
            <div><label class="label">Jumlah Peserta</label><input v-model.number="pax" type="number" min="1" :max="tour.maxPax || 20" class="input" /></div>
            ...
            <div><label class="label">Metode Bayar</label><select v-model="f.method" class="input"><option value="QRIS">QRIS</option><option value="VA_BCA">BCA VA</option><option value="EWALLET_GOPAY">GoPay</option></select></div>
            <div class="flex justify-between font-bold pt-1"><span>Total</span><span class="text-brand-700">{{ rupiah(tour.price * pax) }}</span></div>
            <button @click="book" :disabled="loading" class="btn-brand w-full">{{ loading ? 'Memproses…' : 'Pesan Tour' }}</button>
          ...
          <div v-else-if="doneCode" class="mt-4 text-center">
            ...
            <div class="font-bold">Tour dipesan!</div>
            <div class="text-[13px] text-ink-muted">Kode: {{ doneCode }}</div>
          </div>
          <NuxtLink v-else :to="`/login?redirect=/tours/${tour.id}`" class="btn-brand w-full mt-4">Masuk untuk Memesan</NuxtLink>
        </div>
      </aside>
    </div>
  </div>
  <div v-else class="container-site py-24 text-center text-ink-muted">Tour tidak ditemukan.</div>
</template>
...
async function book() {
  err.value = ''
  if (!f.name.trim() || f.phone.trim().length < 6) { err.value = 'Lengkapi nama & nomor HP.'; return }
  loading.value = true
  try {
  ...
  } catch (e: any) { err.value = e?.data?.error || 'Gagal memesan tour.' }
  finally { loading.value = false }
}
useHead(() => ({ title: tour.value ? `${tour.value.title} · Miruum` : 'Tour · Miruum' }))
</script>
