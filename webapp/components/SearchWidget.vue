<template>
  <div class="bg-white rounded-2xl shadow-pop p-4 sm:p-5">
    ...
    <div class="flex gap-1 overflow-x-auto no-scrollbar mb-4">
      <button class="flex items-center gap-2 px-4 py-2.5 rounded-full font-semibold text-[14px] whitespace-nowrap bg-sky text-white">
    ...
    <form @submit.prevent="go" class="grid gap-2.5 lg:grid-cols-[2fr_1.2fr_1.2fr_auto] items-stretch">
      <!-- Destination -->
      <label class="flex items-center gap-2 border border-line rounded-xl px-3.5 py-3 focus-within:border-sky focus-within:ring-2 focus-within:ring-sky/15">
      ...
      <div class="grid grid-cols-2 gap-2.5">
        <label class="border border-line rounded-xl px-3 py-3 focus-within:border-sky focus-within:ring-2 focus-within:ring-sky/15">
      ...
      <!-- Guests & rooms -->
      <div class="relative">
        <button type="button" @click="gOpen = !gOpen" class="w-full h-full text-left border border-line rounded-xl px-3.5 py-3 hover:border-sky">
        ...
        <div v-if="gOpen" class="absolute z-20 mt-2 right-0 w-64 card shadow-pop p-4 space-y-3">
          <div class="flex items-center justify-between">
            ...
            <div class="flex items-center gap-3">
              <button type="button" @click="guests = Math.max(1, guests-1)" class="w-8 h-8 rounded-full border border-line grid place-items-center hover:border-sky">−</button>
              <span class="w-5 text-center font-semibold">{{ guests }}</span>
              <button type="button" @click="guests = Math.min(16, guests+1)" class="w-8 h-8 rounded-full border border-line grid place-items-center hover:border-sky">+</button>
            </div>
          </div>
          <div class="flex items-center justify-between">
            <span class="text-[14px] font-medium">Kamar</span>
            <div class="flex items-center gap-3">
              <button type="button" @click="rooms = Math.max(1, rooms-1)" class="w-8 h-8 rounded-full border border-line grid place-items-center hover:border-sky">−</button>
              <span class="w-5 text-center font-semibold">{{ rooms }}</span>
              <button type="button" @click="rooms = Math.min(8, rooms+1)" class="w-8 h-8 rounded-full border border-line grid place-items-center hover:border-sky">+</button>
            </div>
          </div>
